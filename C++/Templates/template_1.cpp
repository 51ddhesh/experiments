...
// Vector utilities
template <typename T>
vector<T> make_vector(size_t size, T value) {
    return vector<T>(size, value);
}

template <typename T>
vector<vector<T>> make_2d_vector(size_t rows, size_t cols, T value) {
...
constexpr int ddx[] = {-1, -1, -1, 0, 0, 1, 1, 1};
constexpr int ddy[] = {-1, 0, 1, -1, 1, -1, 0, 1};

void solve() {
    // Example usage of the debug system:
    ...
    PROFILE_END("Loop computation");
    
    // Function debugging
    debugfn("solve");
    
    // Your actual solution code here
